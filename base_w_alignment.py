...
for start_step in range(0, opt.nepoch):
    ...
    errG = center_loss * opt.center_weight + fake_ins_contras_loss * opt.cr - criticG_fake  + FC_loss * opt.cls_weight
    errG.backward()
    optimizerG.step()
    ...
    log_text = 'Iter-{}; errD: {:.3f} errG:{:.3f} gp:{:.3f} center_loss: {:.3f}, c1:{:.3f},c2:{:.3f}, rc:{:.3f} fc:{:.3f} RC:{:.3f}, FC:{:.3f}  ' \
        .format(start_step, D_cost.item(), errG.item(), gradient_penalty.item(), center_loss.item(),
                ...
                fake_ins_contras_loss.item(), RC_loss.item(), FC_loss.item())
    print(log_text)
