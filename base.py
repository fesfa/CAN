...
for start_step in range(0, opt.nepoch):
    ...
    errG =  - criticG_fake + FC_loss * opt.cls_weight
    errG.backward()
    optimizerG.step()
    ...
    log_text = 'Iter-{}; errD: {:.3f} errG:{:.3f} gp:{:.3f} , rc:{:.3f} fc:{:.3f}' \
        .format(start_step, D_cost.item(), errG.item(), gradient_penalty.item(), RC_loss.item(), FC_loss.item())
    print(log_text)
